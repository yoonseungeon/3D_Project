...
HRESULT CMyChannel::Initialize(const myNodeAnim* pMyNodeAnim, class CMyModel* pModel)
{
    ...
    m_iNumKeyFrames = max(pMyNodeAnim->mNumScalingKeys, pMyNodeAnim->mNumRotationKeys);
    m_iNumKeyFrames = max(m_iNumKeyFrames, pMyNodeAnim->mNumPositionKeys);
    ...
    for (size_t i = 0; i < m_iNumKeyFrames; ++i)
    {
        ...
        if (i < pMyNodeAnim->mNumScalingKeys)
        {
            memcpy(&vScale, &pMyNodeAnim->mScalingKeys[i].mValue, sizeof(_float3));
            KeyFrame.fTrackPosition = static_cast<_float>(pMyNodeAnim->mScalingKeys[i].mTime);
        }
        if (i < pMyNodeAnim->mNumRotationKeys)
        {
            // memcpy(&vRotation, &pMyNodeAnim->mRotationKeys[i].mValue, sizeof(_float4));
            vRotation.x = pMyNodeAnim->mRotationKeys[i].mValue.x;
            vRotation.y = pMyNodeAnim->mRotationKeys[i].mValue.y;
            vRotation.z = pMyNodeAnim->mRotationKeys[i].mValue.z;
            vRotation.w = pMyNodeAnim->mRotationKeys[i].mValue.w;
            KeyFrame.fTrackPosition = static_cast<_float>(pMyNodeAnim->mRotationKeys[i].mTime);
        }
        if (i < pMyNodeAnim->mNumPositionKeys)
        {
            memcpy(&vTranslation, &pMyNodeAnim->mPositionKeys[i].mValue, sizeof(_float3));
            KeyFrame.fTrackPosition = static_cast<_float>(pMyNodeAnim->mPositionKeys[i].mTime);
        }
...
void CMyChannel::Update_TransformationMatrix(const vector<CMyBone*>& Bones, _float fCurrentTrackPosition, _uint* pCurrentKeyIndex)
{
...
CMyChannel* CMyChannel::Create(const myNodeAnim* pMyNodeAnim, class CMyModel* pModel)
{
    ...
    if (FAILED(pInstance->Initialize(pMyNodeAnim, pModel)))
    {
        MSG_BOX("Failed to Created: CMyChannel");
