...
CRapier::CRapier(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPartObject{ pDevice, pContext }
{
}

CRapier::CRapier(const CRapier& Prototype)
    : CPartObject{ Prototype }
{
}

HRESULT CRapier::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CRapier::Initialize(void* pArg)
{
    RAPIER_DESC* pDesc = static_cast<RAPIER_DESC*>(pArg);
...
void CRapier::Priority_Update(_float fTimeDelta)
{
}

void CRapier::Parallel_Update(_float fTimeDelta)
{
}

void CRapier::Update(_float fTimeDelta)
{
}

void CRapier::Late_Update(_float fTimeDelta)
{
...
HRESULT CRapier::Ready_Components()
{
    /* For.Com_Shader */
    ...
    /* For.Com_Model */
    if (FAILED(__super::Add_Component(ETOUI(LEVEL::GAMEPLAY), TEXT("Prototype_Component_Model_FioraRapier"),
...
HRESULT CRapier::Bind_ShaderResources()
{
    if (FAILED(m_pShaderCom->Bind_Matrix("g_WorldMatrix", &m_CombinedWorldMatrix)))
        return E_FAIL;
...
HRESULT CRapier::Bind_OutLineShaderResources()
{
...
CRapier* CRapier::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CRapier* pInstance = new CRapier(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Created: CRapier");
...
CGameObject* CRapier::Clone(void* pArg)
{
    CRapier* pInstance = new CRapier(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned: CRapier");
        Safe_Release(pInstance);
    }
