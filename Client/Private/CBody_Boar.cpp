...
CBody_Boar::CBody_Boar(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPartObject{ pDevice, pContext }
{
}

CBody_Boar::CBody_Boar(const CBody_Boar& Prototype)
    : CPartObject{ Prototype }
{
}

HRESULT CBody_Boar::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CBody_Boar::Initialize(void* pArg)
{
    BODY_BOAR_DESC* pDesc = static_cast<BODY_BOAR_DESC*>(pArg);
...
void CBody_Boar::Priority_Update(_float fTimeDelta)
{

}

void CBody_Boar::Parallel_Update(_float fTimeDelta)
{
    m_pModelCom->Play_Animation(fTimeDelta);
}

void CBody_Boar::Update(_float fTimeDelta)
{
}

void CBody_Boar::Late_Update(_float fTimeDelta)
{
...
HRESULT CBody_Boar::Render_Shadow()
{
...
HRESULT CBody_Boar::Render_OutLine()
{
    if (FAILED(Bind_ShaderResources()))
        return E_FAIL;
...
const _float4x4* CBody_Boar::Get_BoneMatrixPtr(const _char* pBoneName) const
{
    return m_pModelCom->Get_BoneMatrixPtr(pBoneName);
}

HRESULT CBody_Boar::Ready_Components()
{
    /* For.Com_Shader */
    ...
    /* For.Com_Model */
    if (FAILED(__super::Add_Component(ETOUI(LEVEL::GAMEPLAY), TEXT("Prototype_Component_Model_Boar"),
...
HRESULT CBody_Boar::Bind_ShaderResources()
{
...
HRESULT CBody_Boar::Bind_OutLineShaderResources()
{
...
CBody_Boar* CBody_Boar::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CBody_Boar* pInstance = new CBody_Boar(pDevice, pContext);
...
CGameObject* CBody_Boar::Clone(void* pArg)
{
    CBody_Boar* pInstance = new CBody_Boar(*this);
