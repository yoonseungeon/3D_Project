...
CBody_Bear::CBody_Bear(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    : CPartObject{ pDevice, pContext }
{
}

CBody_Bear::CBody_Bear(const CBody_Bear& Prototype)
    : CPartObject{ Prototype }
{
}

HRESULT CBody_Bear::Initialize_Prototype()
{
    return S_OK;
}

HRESULT CBody_Bear::Initialize(void* pArg)
{
    BODY_BEAR_DESC* pDesc = static_cast<BODY_BEAR_DESC*>(pArg);
...
void CBody_Bear::Priority_Update(_float fTimeDelta)
{

}

void CBody_Bear::Parallel_Update(_float fTimeDelta)
{
    m_pModelCom->Play_Animation(fTimeDelta);
}

void CBody_Bear::Update(_float fTimeDelta)
{
}

void CBody_Bear::Late_Update(_float fTimeDelta)
{
...
HRESULT CBody_Bear::Render_Shadow()
{
...
HRESULT CBody_Bear::Render_OutLine()
{
    if (FAILED(Bind_ShaderResources()))
        return E_FAIL;
...
const _float4x4* CBody_Bear::Get_BoneMatrixPtr(const _char* pBoneName) const
{
    return m_pModelCom->Get_BoneMatrixPtr(pBoneName);
}

HRESULT CBody_Bear::Ready_Components()
{
    /* For.Com_Shader */
    ...
    /* For.Com_Model */
    if (FAILED(__super::Add_Component(ETOUI(LEVEL::GAMEPLAY), TEXT("Prototype_Component_Model_Bear"),
...
HRESULT CBody_Bear::Bind_ShaderResources()
{
...
HRESULT CBody_Bear::Bind_OutLineShaderResources()
{
...
CBody_Bear* CBody_Bear::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CBody_Bear* pInstance = new CBody_Bear(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed to Created: CBody_Bear");
...
CGameObject* CBody_Bear::Clone(void* pArg)
{
    CBody_Bear* pInstance = new CBody_Bear(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned: CBody_Bear");
        Safe_Release(pInstance);
    }
